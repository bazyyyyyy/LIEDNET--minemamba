...
# Import MineGate fusion module
try:
    from basicsr.archs.minegate_fusion import MineGateFusion, SimpleMineGateFusion
    HAS_MINEGATE = True
except:
    HAS_MINEGATE = False
    print("Warning: MineGateFusion not found, using default fusion")

# Import new modules for replacement
try:
    from module.vss_block import VSSBlock
    from module.modules import ChannelMambaBlock, DepthwiseConv2d
    HAS_NEW_MODULES = True
except:
    HAS_NEW_MODULES = False
    print("Warning: New modules (VSSBlock, ChannelMambaBlock) not found, using default modules")
...
class LocalFeatureModule(nn.Module):
    ...
    def forward(self, x):
        ...
        channel_att = self.channel_weight(channel_att)
        
        # 空间注意力（新增）
        spatial_att = self.spatial_weight(x)
...
class VSSLocalBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if self.ssm_branch:
            ...
            # 核心创新：使用 VSSBlock 替换 SS2D（VSSM）
            # VSSBlock 对应架构图中的 VSSM，支持4方向扫描
            use_vssblock = kwargs.get('use_vssblock', False)  # 默认使用 SS2D，可通过参数启用
            
            if use_vssblock and HAS_NEW_MODULES:
                # 使用新的 VSSBlock（来自 module2_illumination）
                self.op = VSSBlock(
                    dim=hidden_dim,
                    d_state=ssm_d_state,
                    ssm_ratio=ssm_ratio,
                    ...
                    drop_path=drop_path,
                    scan_direction='all',  # 4方向扫描
                    channel_first=channel_first,
                )
                self.use_vssblock = True
            else:
                # 使用原始的 SS2D
                self.op = SS2D(
                    d_model=hidden_dim, 
                ...
                self.use_vssblock = False
        
        self.drop_path = DropPath(drop_path)
        
        # 核心创新：使用 ChannelMambaBlock 替换 LocalFeatureModule（LFM）
        # ChannelMambaBlock 对应架构图中的 LFM，通道维度的 Mamba 更轻量
        use_channel_mamba = kwargs.get('use_channel_mamba', False)  # 默认使用 LocalFeatureModule
        
        if use_channel_mamba and HAS_NEW_MODULES:
            # 使用新的 ChannelMambaBlock（来自 module3_reflectance）
            self.local = ChannelMambaBlock(d_model=hidden_dim)
            self.use_channel_mamba = True
        else:
            # 使用原始的 LocalFeatureModule
            self.local = LocalFeatureModule(dim=hidden_dim)
            self.use_channel_mamba = False
        
        # 核心创新：使用 MineGate 实现双分支自适应融合
        # 替换原来的简单 concat + 1×1 conv 融合
        use_minegate = kwargs.get('use_minegate', True)  # 默认启用 MineGate
        if use_minegate and HAS_MINEGATE:
            # 使用 MineGate 自适应融合：全局VSSM vs 局部LFM
            # 物理先验：极暗信全局，局部强光信局部
            self.fusion = SimpleMineGateFusion()  # 简化版，直接从特征提取统计量
            self.use_minegate = True
        else:
            # 回退到原始融合方式
            self.fusion = nn.Conv2d(2*hidden_dim, hidden_dim, 1, 1, 0)
            self.use_minegate = False
        

        if self.use_ffn:
            self.norm2 = norm_layer(hidden_dim)
            
            # 可选：使用 DepthwiseConv2d 优化 DGDFFN（来自 module3_reflectance）
            use_depthwise_ffn = kwargs.get('use_depthwise_ffn', False)  # 默认使用 DGDFFeedForward
            
            if use_depthwise_ffn and HAS_NEW_MODULES:
                # 使用 DepthwiseConv2d 优化的 FFN（简化版）
                # 注意：这里只是示例，实际可以基于 DepthwiseConv2d 重新设计 FFN
                self.ffn = DGDFeedForward(dim=hidden_dim, ffn_expansion_factor=2.66, bias=False)
                self.use_depthwise_ffn = False  # 暂时保持原样，后续可以优化
            else:
                # 使用原始的 DGDFFeedForward（已经实现得很好）
                self.ffn = DGDFeedForward(dim=hidden_dim, ffn_expansion_factor=2.66, bias=False)
                self.use_depthwise_ffn = False
        else:        
            if self.mlp_branch:
    ...
    def _forward(self, input: torch.Tensor):
        ...
        # 双分支：全局VSSM vs 局部LFM
        if self.use_vssblock:
            # VSSBlock 输出格式可能不同，需要适配
            if self.channel_first:
                x_global = self.drop_path(self.op(x_norm.permute(0, 3, 1, 2).contiguous()))  # (B, C, H, W)
            else:
                x_global = self.drop_path(self.op(x_norm)).permute(0, 3, 1, 2).contiguous()  # (B, C, H, W)
        else:
            # 原始 SS2D
        ...
        if self.use_channel_mamba:
            # ChannelMambaBlock 输入是 (B, C, H, W)
            x_local = self.local(x_norm.permute(0, 3, 1, 2).contiguous())  # (B, C, H, W)
        else:
            # 原始 LocalFeatureModule
            x_local = self.local(x_norm.permute(0, 3, 1, 2).contiguous())  # (B, C, H, W)
        
        # 核心创新：MineGate 自适应融合
        # 物理先验驱动：极暗信全局，局部强光信局部
        if self.use_minegate:
            # MineGate 融合：alpha * F_global + (1-alpha) * F_local
            x_fused = self.fusion(x_global, x_local)  # (B, C, H, W)
            x = x_fused.permute(0, 2, 3, 1).contiguous() + input  # (B, H, W, C)
        else:
            # 原始融合方式：concat + 1×1 conv
