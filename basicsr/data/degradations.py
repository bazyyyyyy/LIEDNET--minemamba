...
try:
    ...
    def rgb_to_grayscale(img, num_output_channels=1):
        ...
        Args:
            img (Tensor): RGB image tensor, shape (B, C, H, W) or (C, H, W)
            num_output_channels (int): Number of output channels. Default: 1.
        Returns:
            Tensor: Grayscale image tensor
        """
        if img.dim() == 3:
            img = img.unsqueeze(0)
        
        # RGB to grayscale conversion weights (ITU-R BT.601)
        r, g, b = img[:, 0:1], img[:, 1:2], img[:, 2:3]
        gray = 0.299 * r + 0.587 * g + 0.114 * b
        
        if num_output_channels > 1:
            gray = gray.repeat(1, num_output_channels, 1, 1)
